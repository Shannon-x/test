...
window.CONFIG = {
  ...
  page: {
    ...
    navbar: {
      ...
      important: ["dashboard", "shop", "profile"],
      more: true, // 是否显示更多菜单
    },
  ...
  home: {
    ...
    nodes: {
      country: [
        ...
        "zm",
        "zw",
      ], // 节点国家代码，用于显示国旗，ISO 3166-1 alpha-2，可以根据需要删减，国家名称在国际化配置
    },
    // 首页每一个模块的显示配置
    display: {
      poster: true,
  ...
  order: {
    ...
    url: {
      ...
      mobile: true, // 是否在移动端显示支付链接，如果为 true，则会新窗口自动打开链接
      miniWindow: true, // 是否在小窗口中显示支付链接，如果为 true，则会在小窗口中打开链接，这会有效防止用户离开您的站点页面
    },
    qrcode: {
      pc: false, // 是否在 PC 端显示支付链接二维码
      mobile: false, // 是否在移动端显示支付链接二维码
    },
  },
    ...
    special: {
      ...
      presets: [
        // 面向所有用户的弹窗
        {
          ...
          countdown: {
            startTime: new Date("2025-09-15").getTime(),
            effectiveTime: 48,
          },
        },
        // 面向新用户的弹窗
  ...
  recharge: {
    ...
    customInput: false, // 是否允许用户自定义输入充值金额
    min: 100,
    max: 9999,
    float: false,
  },
  ...
  quick: {
    ...
    password: {
      random: true, // 是否随机生成密码，true 为随机生成，false 为不随机生成，false 时，密码输入框将会显示在购买页面，不受 custom 影响
      custom: true, // 是否允许用户自定义输入密码，true 为允许，false 为不允许
    },
  },
